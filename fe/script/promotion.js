...
async function createDiscount() {
    ...
    try {
        ...
        if (response.ok && result.status === "success") {
            ...
            console.log('result >>>', result);
            document.getElementById('summaryCode').textContent = result.id || "N/A"; // Assuming API returns an ID
            document.getElementById('summaryDescription').textContent = requestBody.content;
            document.getElementById('summaryStartDate').textContent = requestBody.start_date;
            document.getElementById('summaryEndDate').textContent = requestBody.end_date;
            ...
            // Handle API errors
            alert(`Tạo khuyến mãi thất bại: ngày bắt đầu không được sau ngày kết thúc`);
        }
    } catch (error) {
...
async function applyDiscount() {
    ...
    try {
        ...
        if (response.ok && result.status === "success") {
            ...
            document.getElementById("applySummaryCode").textContent = requestBody.promotionID;
            document.getElementById("applySummaryProduct").textContent = requestBody.productID;
            document.getElementById("applySummaryDiscount").textContent = requestBody.discountRate;
            document.getElementById("applySummaryCondition").textContent = requestBody.useCondition;
            ...
            // Handle API errors
            alert(`Áp dụng khuyến mãi thất bại: ${result.message || "Unknown error"}`);
        }
...
async function updatePromotion() {
    ...
    try {
        ...
        if (response.ok && result.status === "Success") {
            ...
            alert(`Cập nhật thông tin khuyến mãi thất bại: ${result.message || "Unknown error"}`);
        }
    } catch (error) {
...
async function deletePromotion() {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/promotion/delete?promotionID=${promotionID}`, {
            method: "DELETE",
        });
